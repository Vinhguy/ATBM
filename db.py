...
def register():
    if request.method == 'POST':
        ...
        try:
            cursor.execute(
                "INSERT INTO users (username, salt, encrypted_password) VALUES (%s, %s, %s)",
...
def login():
    if request.method == 'POST':
        ...
        cursor.execute(
            "SELECT salt, encrypted_password, fail_attempts, is_locked FROM users WHERE username = %s",
            (username,)
        )
        ...
        salt, stored_encrypted_password, fail_attempts, is_locked = user
...
def admin():
    # Giả sử chỉ admin có quyền truy cập (thêm logic xác thực admin sau)
